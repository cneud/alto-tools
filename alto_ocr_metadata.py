...
if xmlns in namespace.values():
    ...
    sys.stdout.write('\n<Description>\n')
    try:
        tree.find('.//{%s}Description' % xmlns).find('{%s}sourceImageInformation' % xmlns).find('{%s}fileName' % xmlns).text != None
    ...
    try:
        tree.find('.//{%s}Description' % xmlns).find('{%s}sourceImageInformation' % xmlns).find('{%s}fileIdentifier' % xmlns).text != None
    ...
    try:
        tree.find('.//{%s}Description' % xmlns).find('{%s}sourceImageInformation' % xmlns).find('{%s}documentIdentifier' % xmlns).text != None
    ...
    try: 
        tree.find('.//{%s}Description' % xmlns).find('{%s}MeasurementUnit' % xmlns).text != None
    ...
    sys.stdout.write('\n\n<OCRProcessing>\n')
    try: 
        tree.find('.//{%s}OCRProcessing' % xmlns).text != None
    ...
    sys.stdout.write('\n\n<preProcessingStep>\n')
    try: 
        tree.find('.//{%s}preProcessingStep' % xmlns).find('{%s}processingDateTime' % xmlns).text != None
    ...
    try: 
        tree.find('.//{%s}preProcessingStep' % xmlns).find('{%s}processingAgency' % xmlns).text != None
        sys.stdout.write('\nprocessingAgency           =   %s' % tree.find('.//{%s}preProcessingStep' % xmlns).find('{%s}processingAgency' % xmlns).text)
    except AttributeError:
        sys.stdout.write('\nprocessingAgency           =   -- NOT_DEFINED --')
    try: 
        tree.find('.//{%s}preProcessingStep' % xmlns).find('{%s}processingStepDescription' % xmlns).text != None
    ...
    try: 
        tree.find('.//{%s}preProcessingStep' % xmlns).find('{%s}processingStepSettings' % xmlns).text != None
    ...
    try: 
        tree.find('.//{%s}preProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}softwareCreator' % xmlns).text != None
    ...
    try: 
        tree.find('.//{%s}preProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}softwareName' % xmlns).text != None
    ...
    try: 
        tree.find('.//{%s}preProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}softwareVersion' % xmlns).text != None
    ...
    try: 
        tree.find('.//{%s}preProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}applicationDescription' % xmlns).text != None
    ...
    sys.stdout.write('\n\n<ocrProcessingStep>\n')
    try: 
        tree.find('.//{%s}ocrProcessingStep' % xmlns).find('{%s}processingDateTime' % xmlns).text != None
        sys.stdout.write('\nprocessingDateTime         =   %s' % tree.find('.//{%s}ocrProcessingStep' % xmlns).find('{%s}processingDateTime' % xmlns).text)
    except AttributeError:
        sys.stdout.write('\nprocessingDateTime         =   -- NOT_DEFINED --')
    try: 
        tree.find('.//{%s}ocrProcessingStep' % xmlns).find('{%s}processingAgency' % xmlns).text != None
        sys.stdout.write('\nprocessingAgency           =   %s' % tree.find('.//{%s}ocrProcessingStep' % xmlns).find('{%s}processingAgency' % xmlns).text)
    except AttributeError:
        sys.stdout.write('\nprocessingAgency           =   -- NOT_DEFINED --')
    try: 
        tree.find('.//{%s}ocrProcessingStep' % xmlns).find('{%s}processingStepDescription' % xmlns).text != None
        sys.stdout.write('\nprocessingStepDescription  =   %s' % tree.find('.//{%s}ocrProcessingStep' % xmlns).find('{%s}processingStepDescription' % xmlns).text)
    except AttributeError:
        sys.stdout.write('\nprocessingStepDescription  =   -- NOT_DEFINED --')
    try: 
        tree.find('.//{%s}ocrProcessingStep' % xmlns).find('{%s}processingStepSettings' % xmlns).text != None
        sys.stdout.write('\nprocessingStepSettings     =   %s' % tree.find('.//{%s}ocrProcessingStep' % xmlns).find('{%s}processingStepSettings' % xmlns).text)
    except AttributeError:
        sys.stdout.write('\nprocessingStepSettings     =   -- NOT_DEFINED --')
    try: 
        tree.find('.//{%s}ocrProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}softwareCreator' % xmlns).text != None
        sys.stdout.write('\nsoftwareCreator            =   %s' % tree.find('.//{%s}ocrProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}softwareCreator' % xmlns).text)
    except AttributeError:
        sys.stdout.write('\nsoftwareCreator            =   -- NOT_DEFINED --')
    try: 
        tree.find('.//{%s}ocrProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}softwareName' % xmlns).text != None
        sys.stdout.write('\nsoftwareName               =   %s' % tree.find('.//{%s}ocrProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}softwareName' % xmlns).text)
    except AttributeError:
        sys.stdout.write('\nsoftwareName               =   -- NOT_DEFINED --')
    try: 
        tree.find('.//{%s}ocrProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}softwareVersion' % xmlns).text != None
        sys.stdout.write('\nsoftwareVersion            =   %s' % tree.find('.//{%s}ocrProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}softwareVersion' % xmlns).text)
    except AttributeError:
        sys.stdout.write('\nsoftwareVersion            =   -- NOT_DEFINED --')
    try: 
        tree.find('.//{%s}ocrProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}applicationDescription' % xmlns).text != None
    ...
    sys.stdout.write('\n\n<postProcessingStep>\n')
    try: 
        tree.find('.//{%s}postProcessingStep' % xmlns).find('{%s}processingDateTime' % xmlns).text != None
        sys.stdout.write('\nprocessingDateTime         =   %s' % tree.find('.//{%s}postProcessingStep' % xmlns).find('{%s}processingDateTime' % xmlns).text)
    except AttributeError:
        sys.stdout.write('\nprocessingDateTime         =   -- NOT_DEFINED --')
    try: 
        tree.find('.//{%s}postProcessingStep' % xmlns).find('{%s}processingAgency' % xmlns).text != None
        sys.stdout.write('\nprocessingAgency           =   %s' % tree.find('.//{%s}postProcessingStep' % xmlns).find('{%s}processingAgency' % xmlns).text)
    except AttributeError:
        sys.stdout.write('\nprocessingAgency           =   -- NOT_DEFINED --')
    try: 
        tree.find('.//{%s}postProcessingStep' % xmlns).find('{%s}processingStepDescription' % xmlns).text != None
        sys.stdout.write('\nprocessingStepDescription  =   %s' % tree.find('.//{%s}postProcessingStep' % xmlns).find('{%s}processingStepDescription' % xmlns).text)
    except AttributeError:
        sys.stdout.write('\nprocessingStepDescription  =   -- NOT_DEFINED --')
    try: 
        tree.find('.//{%s}postProcessingStep' % xmlns).find('{%s}processingStepSettings' % xmlns).text != None
        sys.stdout.write('\nprocessingStepSettings     =   %s' % tree.find('.//{%s}postProcessingStep' % xmlns).find('{%s}processingStepSettings' % xmlns).text)
    except AttributeError:
        sys.stdout.write('\nprocessingStepSettings     =   -- NOT_DEFINED --')
    try: 
        tree.find('.//{%s}postProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}softwareCreator' % xmlns).text != None
        sys.stdout.write('\nsoftwareCreator            =   %s' % tree.find('.//{%s}postProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}softwareCreator' % xmlns).text)
    except AttributeError:
        sys.stdout.write('\nsoftwareCreator            =   -- NOT_DEFINED --')
    try: 
        tree.find('.//{%s}postProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}softwareName' % xmlns).text != None
        sys.stdout.write('\nsoftwareName               =   %s' % tree.find('.//{%s}postProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}softwareName' % xmlns).text)
    except AttributeError:
        sys.stdout.write('\nsoftwareName               =   -- NOT_DEFINED --')
    try: 
        tree.find('.//{%s}postProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}softwareVersion' % xmlns).text != None
        sys.stdout.write('\nsoftwareVersion            =   %s' % tree.find('.//{%s}postProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}softwareVersion' % xmlns).text)
    except AttributeError:
        sys.stdout.write('\nsoftwareVersion            =   -- NOT_DEFINED --')
    try: 
        tree.find('.//{%s}postProcessingStep' % xmlns).find('{%s}processingSoftware' % xmlns).find('{%s}applicationDescription' % xmlns).text != None
